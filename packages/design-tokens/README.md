...
Change `--blue-brand-lch` once and every derived shade re-themes automatically. The same idiom is used in the commerce platform's storefront-kit; we intentionally rhyme.
...
All variables are prefixed `--blue-*` so they never collide with merchant-supplied tokens on the storefront, the platform design system tokens in the admin marketplace shell, or BC platform tokens elsewhere.
