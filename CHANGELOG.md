...
- **Portable `BLUEPRINT_HOME` resolution.** The SessionStart hook resolves the methodology source via `$BLUEPRINT_HOME` → `blueprint.yml` `methodology_home:` → local canonical path (`~/Workspace/dev/tools/blueprint`) → npm-installed `@nino-chavez/blueprint-cli`; a candidate counts only if it contains `METHODOLOGY.md`. Removes the single hardcoded `~/Workspace/dev/wip/blueprint` default. A team member who `npm install`s the CLI resolves with zero config.
