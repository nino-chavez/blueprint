...
|---|---|---|
| **`separate`** | Each tool app is its own deploy (current `bc-subscriptions` pattern) | Existing projects with deployed URLs other things already link to; teams that want stack freedom per surface; surfaces with fundamentally different rebuild cadences |
...
- Existing projects with deployed URLs other things link to (e.g., `bc-subscriptions` — three surfaces are linked from memory entries, Slack, external docs; migration cost > maintenance benefit)
