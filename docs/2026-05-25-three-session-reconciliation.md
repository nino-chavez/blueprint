...
- **Variant pill naming.** bc-subscriptions uses `executive / discovery / internal`. "Internal" is overloaded (internal to what?). ADR candidate to rename `internal` → `engineering` or keep as-is.
