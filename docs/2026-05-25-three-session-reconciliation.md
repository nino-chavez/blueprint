...
**Date**: 2026-05-25
**Trigger**: Nino observed drift across four Blueprint reference projects (`subs-initiative`, `apps/website-nc-v3`, `apps/rally-hq`, `apps/blog`) and asked three independent Claude Code sessions — running in different project repos — the same question: "step back and determine the goal and intent of blueprint and the portal and shell approach. then how do we make it consistent when blueprint is applied on any project or idea new or existing."
...
| **Website-nc-v3 session** (this doc's author) | `apps/website-nc-v3` | Cross-repo audit of all four projects + extraction of subs-initiative canonical packages | Saw the v1 → v2 portal jump subs-initiative already made; defined the tier ladder; landed the IA contract |
...
`subs-initiative` is at Tier 2. The Rally HQ and Blog sessions were proposing improvements to Tier 0 without realizing Tier 1 had already been built and shipped. Both sets of improvements remain valid — they apply to Tier 0 surfaces, where variant-walking design-principles deliberation correctly belongs.
...
**Website-nc-v3 session** did: extract subs-initiative' `apps/portal/`, `packages/ui/`, `packages/design-tokens/` into `template/apps/portal/`, `template/packages/ui/`, `template/packages/design-tokens/` with `@blueprint/*` rename.
...
- Tier 1 layer (`template/apps/portal/` + `template/packages/`) gets the subs-initiative canonical extraction.
...
- **subs-initiative already built the next-generation shell** (`apps/portal/` Astro + `@subs-initiative/ui` + `@subs-initiative/design-tokens`) — neither other session knew this.
...
1. **Tier 1 canonical extraction** (done — this session). `template/apps/portal/` + `template/packages/ui/` + `template/packages/design-tokens/` lifted from subs-initiative.
...
4. **`template/prototype/PORTAL.md`** with the "one confident preview, not a deliberation venue" rule, enforced via extended `design-principles-reviewer` (Blog session's responsibility). Stage 2 → 3 gate. **→ SHIPPED (wave 1, see line 176) as [`template/docs/methodology/confident-preview-rule.md`](../template/docs/methodology/confident-preview-rule.md)** — a methodology doc rather than a `prototype/PORTAL.md`, so it sits beside `methodology-amendments-convention.md`. Enforcement landed at the Stage 2→3 boundary (`design-principles-reviewer` greps planned page names) AND Stage 3 (`portal-pattern-a`/`-b-conformance-reviewer` grep shipped page names) — deliberately NOT in `prototype-smoke-runner`, because the prototype/ scratch tier is where Stage-2 variant deliberation legitimately belongs.
...
The subs-initiative Astro shell is **Pattern A** — the platform-portal, with its own canonical primitives (6-verb IA + audience switcher + LaneCard + StatusBadge + LiveIframe).
...
- `template/apps/portal/` is the canonical Pattern A template (extracted from subs-initiative)
...
- The four reference projects fall into clearer rows:
  - `subs-initiative`: Pattern A, Tier 2, canonical
...
- **2026-05-25 evening, wave 4**: Pattern B chrome storage-key + identifier audit. Applied ADR-0002 convention (`blueprint-` prefix) to `template/portal/index.html`, `proto-nav.js`, `proto-annotate.js`. Footer + chat.js OpenRouter attribution made manifest-driven. ADR-0002 addendum records the parallel rename. After this wave, `grep -rE 'rally|nino|chavez|subs-initiativecription' template/portal/` returns only documentation and historical-comment references in code-path files.
