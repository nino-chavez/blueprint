...
Every archaeological need on `subs-initiative` produced a new tool:
...
- `{kind: "fetched_url",    target: "url:https://the platform vendor docs/..."}`
...
- Commit subjects: `closes #N` AND `(synthesis #N)` when applicable (already required on `subs-initiative`)
...
Captured from the subs-initiative production hydration so the next project doesn't re-discover them:
...
`subs-initiative` (May 2026) is the canonical retrofit case study. [`tools/archaeology/`](the subscriptions initiative's repo (private)/tree/dev/tools/archaeology) is the working implementation that this template was derived from:
...
Hydration runbook: [`docs/runbooks/archaeology-hydration.md`](the subscriptions initiative's repo (private)/blob/dev/docs/runbooks/archaeology-hydration.md) — concrete CF provisioning + backfill commands, idempotent and resumable.
...
## Retrofit Sequence (for existing projects, like subs-initiative)
...
`subs-initiative` is the canonical retrofit case study — design doc + runbook capture the full sequence.
...
The subs-initiative retro that produced this pattern surfaced three reusable lessons:
