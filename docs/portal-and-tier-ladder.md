...
- **A bespoke portal is allowed, but ADR-mandatory.** Record why neither A nor B fits, per the Divergence rule below. Silence is the failure mode — it produces unversioned IA that drifts from the next same-archetype consumer (the exact problem the two named patterns exist to prevent; see "Why this exists"). **Declare it**: set `portal_pattern: bespoke` in `blueprint.yml`. **Enforced (wave 48)**: `blueprint doctor` accepts a bespoke portal *iff* a divergence ADR is present in `decisions/` (filename signalling portal + bespoke/divergence) and FAILs `portal-conformance` without one. The second bespoke instance — the methodology's own product-site portal (`decisions/02-portal-bespoke-product-site.md`, a product homepage, a *different* archetype from the ops console) — triggered this automation.
