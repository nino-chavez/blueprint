...
The previous methodology drift used "tier" and "variant" interchangeably (Tier 0 read as "the greenfield discovery phase"). They are not the same axis. A brownfield audit can be at Tier 0 (pure docs) or Tier 1 (Review Portal redesign-review portal). A midstream initiative is never at Tier 0.

## Choosing between Initiative Portal and Review Portal

Pick before scaffolding. Wrong choice produces retrofit feel that costs a full rebuild.

### Initiative Portal — platform-portal
...
### Review Portal — redesign-review-portal
...
├── Yes → Review Portal (redesign-review-portal)
...
└── No
    └── Multiple distinct surfaces and multiple audiences, AS a stakeholder
        ...
        ├── Yes → Initiative Portal (platform-portal)
...
Initiative Portal and Review Portal can coexist in the same project if the surfaces are genuinely separate (e.g., a platform that also publishes audit docs). That's a Tier 2 monorepo with both `apps/portal/` (initiative type) and `portal/` (review type). Rare. Document via ADR.

### When neither pattern fits the archetype (bespoke-with-ADR)

Both named patterns assume **stakeholder-facing externalization**: A is a front-door *to a product*, B is a review surface *for a redesign*. An initiative can be portal-shaped — a front door over multiple surfaces — yet fit neither. The clearest case is an **operator-facing process console**: a control surface for running a recurring operation, whose audience is the team *operating* it, not stakeholders *evaluating* a product. Initiative Portal already carries this concern as one of its six verbs (Operate); a process console is the degenerate case where Operate *is* the product and Discover/Try/Build/Roadmap collapse.

When that happens:

- **A bespoke portal is allowed, but ADR-mandatory.** Record why neither portal type fits, per the Divergence rule below. Silence is the failure mode — it produces unversioned IA that drifts from the next same-archetype consumer (the exact problem the two named patterns exist to prevent; see "Why this exists"). **Declare it**: set `portal_type: bespoke` in `blueprint.yml`. **Enforced (wave 48)**: `blueprint doctor` accepts a bespoke portal *iff* a divergence ADR is present in `decisions/` (filename signalling portal + bespoke/divergence) and FAILs `portal-conformance` without one. The second bespoke instance — the methodology's own product-site portal (`decisions/02-portal-bespoke-product-site.md`, a product homepage, a *different* archetype from the ops console) — triggered this automation.
...
- **The archetype is not yet canonized.** The first instance is `ai-content-engine` (a daily-content operation; bespoke `prototype/` cockpit, `decisions/0001-bespoke-portal-ops-console.md`). One instance names the gap; it does not freeze a contract — Review Portal was extracted from three consumers, not one. A third canonical pattern ("operator console") waits for a *second* instance to define it. Tracked under Open questions.

## Initiative Portal — The IA contract (mandatory)

Every Initiative Portal exposes the same six verbs and the same audience switcher. Stack is project choice (Astro, SvelteKit, Next.js — fine, with `@blueprint/ui-svelte` etc. for non-React stacks). Routes and audience-switcher behavior are not.
...
Each Initiative Portal route has a default voice mode declared at the IA level. Pages under that route inherit it unless the page frontmatter overrides. The mode determines cadence, hedge tolerance, sentence shape, and which voice guide applies.
...
### Initiative Portal is not a deliberation venue
...
## Review Portal — The drawer contract (mandatory)

Every Review Portal exposes the same per-prototype-page chrome. Routes mirror the redesigned product's surface tree; the contract is the shell, not the IA.
...
### Review Portal is also not a deliberation venue

Same rule as Initiative Portal: one confident preview per route. The COMPARE toggle is the comparison primitive; multiple A/B variants of the same page are not. If you have two competing redesign approaches, walk them in `prototype/` (Tier 0), pick one, ship that in the portal.

### Review Portal deployment

The Review Portal deploys as a separate Cloudflare Pages project (e.g., `ninochavez-blueprint` paired with `ninochavez-main`). Zero build pipeline — static HTML + Pages Functions. Custom domain typical (`blueprint.<host>` paired with the redesigned product at `<host>`).
...
### Initiative Portal canonical packages
...
### Review Portal canonical shell

The Review Portal shell lives at `template/portal/` and is consumed by **copy-stamp**, not workspace-link. The static-HTML shell is deliberately zero-build — adding a dependency-resolution step defeats the "deploys in one push" property that makes Review Portal fast.
...
- Initiative Portal with SvelteKit instead of Astro+React → cite `@blueprint/ui-svelte` (when it ships) or shim against `@blueprint/design-tokens`
- Review Portal with build pipeline (e.g., add a CSS bundler) → cite the performance or DX gain that justifies leaving zero-build
- Review Portal replacing strategy drawer with side-by-side full-screen panels → cite the audit shape that requires it
...
- A portal exists. Either Initiative Portal (`apps/portal/`) or Review Portal (`portal/`) — pick by the decision tree above.
...
- Initiative Portal Tier 2: `apps/portal/` + `apps/<product>/`. The portal links to product surfaces; it does not contain them.
- Review Portal Tier 2: `portal/` + `apps/<product>/` (or `src/` for single-app projects). The portal reviews the product; the product ships separately.
- The subscriptions initiative is the canonical Initiative Portal Tier 2 reference.
- `apps/website-nc-v3` is the canonical Review Portal Tier 2 reference (post-graduation; currently on the redesign branch).
...
3. **Audit the current shell against the canonical for the chosen pattern.**
   - Initiative Portal: are the 6 IA routes present? Is `@blueprint/ui` imported (not copy-pasted)? Audience switcher rendered?
   - Review Portal: are drawers, comparison toggle, chat FAB, I-2/I-3/I-5 invariants honored? Is `_meta/<page-id>.json` populated per page?
4. **Fix only what diverges.** Review Portal projects that path-drifted (e.g., `blueprint/prototype/` instead of `blueprint/portal/`) get renamed in place. Initiative Portal projects that are still on Tier 0 static HTML get a separate apps/portal/ Astro app.
...
|---|---|---|---|
| the subscriptions initiative | Initiative Portal — canonical | Tier 2 | None. This is the source of `@blueprint/ui` + `@blueprint/design-tokens` + `apps/portal/`. |
| `apps/website-nc-v3` | Review Portal — canonical (per ADR-0008) | Tier 2 (in flight on redesign branch) | None for the portal pattern itself. Continue ADR-0008's paired-deploy buildout. Optional follow-up: convert Review Portal static HTML to Astro consuming `@blueprint/design-tokens` (keeps Review Portal chrome, gains token consistency) — future ADR. |
| `apps/rally-hq` | Review Portal — canonical | Tier 2 | Rename `blueprint/prototype/` → `blueprint/portal/` (path drift only). Otherwise conformant. |
| `apps/blog` | Review Portal — pre-pivot (still on v1 `_providers.js` + `_shell.css`; the v1→v2 shell upgrade hasn't landed) | Tier 1 → Tier 2 | Two paths: upgrade to v2 Review Portal shell (drawers + toggle + chat), or graduate past the portal entirely if the redesign is shipping at `astro-build/` and no audit review is needed. The blog session is currently rebuilding from variant-walking v1 to confident-preview v2 — that work covers the upgrade. |
...
- The earlier draft of this doc (2026-05-25, earlier today) that collapsed both patterns into a single tier ladder over Initiative Portal only
...
- **Initiative Portal conformance reviewer**: ~~the existing `portal-shell-conformance-reviewer` checks Initiative Portal only. A parallel Review Portal reviewer is required.~~ **Resolved 2026-05-25.** Both reviewers ship in `template/.claude/agents/blueprint/reviewers/portal-initiative-conformance-reviewer.md` and `portal-review-conformance-reviewer.md`. Wired into the reviewer roster as Stage 3 + portal-touching-commit gates.
- **Review Portal Astro variant**: should Review Portal keep static HTML or move to Astro consuming `@blueprint/design-tokens`? Trade-off: zero-build property vs. design-token consistency with Initiative Portal. Future ADR.
- **`@blueprint/ui-svelte` parity**: when does the React-only `@blueprint/ui` get a Svelte equivalent for Initiative Portal SvelteKit consumers? Demand-driven.
...
- **Pattern coexistence**: when a project genuinely needs both A and B portals (multi-audience platform that *also* publishes audit work), how does the workspace plumb both? Single project with `apps/portal/` (initiative type) and `portal/` (review type)? Future ADR.
