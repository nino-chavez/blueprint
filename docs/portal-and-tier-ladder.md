...
The voice taxonomy comes from `signal-forge/docs/voice/` (when Signal Forge is enabled) or `template/docs/voice-template.md` (Blueprint's built-in fallback). Per-page override: add `voice_mode: <name>` to the page's frontmatter. The override is rare — most pages on a route share the route's mode.
