...
Blueprint must work for **three distinct project lifecycles**, each with its own variant. Currently only one is formally codified and one is emerging — the third is missing entirely, and the absence is what produced the retrofit pattern across Rally HQ, website-nc-v3, and subs-initiative.
...
| **Midstream** | Active product, mid-development. Prototype extends or revises something in flight. | Hybrid: lightweight diagnose against current state + prototype against the gap | **Missing.** Rally HQ + subs-initiative both needed this; both got force-fit into greenfield pattern; both produced retrofit feel. |
...
| subs-initiative | `wip/subs-initiative/` | Not yet touched; gated until next active session |
...
These rules become a `tools/blueprint/docs/variant-selection.md` doc, the way `browser-legibility.md` is the canonical Stage 0 reference.
...
Definitions live in `tools/blueprint/template/.claude/agents/`. The three open questions from the v2 patch (location, runtime, smoke-flake policy) need to be resolved here — they were deferred in the predecessor session.
...
- **subs-initiative**: gated until next active session; will use whichever variant fits when it reopens.
...
5. **Draft reviewer agents** under `tools/blueprint/template/.claude/agents/`. Start with `research-completeness-reviewer` because it's the one that would have caught the blog Stage 1 failure.
...
7. **Re-apply variants to consumers** in order: blog (cleanup, mostly mechanical) → Rally HQ (midstream variant adoption) → website-nc-v3 (formalize brownfield) → subs-initiative (gated; defer).
