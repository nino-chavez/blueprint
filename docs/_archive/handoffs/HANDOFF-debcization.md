...
This `apps/portal/` directory was extracted from `bc-subscriptions` at `origin/main` on 2026-05-25 as the canonical Tier 1 portal scaffold. The mechanical `@bc-subscriptions/*` → `@blueprint/*` package rename is complete. The remaining `bc-subscriptions` strings are surface-level project-name + repo-URL references that need to be parameterized before this scaffold is ready for new initiatives.

## What's still bc-subs-specific

Run from `template/apps/portal/`:

```bash
grep -rln "bc-subscriptions" .
```
...
Hard-coded `"bc-subscriptions"` appears as:

- Page title in `src/pages/index.astro` (`title="bc-subscriptions"`)
- Layout title concatenation in `src/layouts/Layout.astro` (`title === 'bc-subscriptions' ? title : ...`)
...
Hard-coded `https://github.com/nino-chavez/bc-subscriptions` appears as:
...
`public/subs-logo.png` is the bc-subscriptions logo. Layout.astro references it via `<img src="/subs-logo.png">`.

**Fix:** rename to `public/project-logo.png`. Update Layout.astro reference. Logo asset is replaceable per project.

### 4. Description copy

Footer copy in `src/layouts/Layout.astro` says "BigCommerce-native subscription management" — bc-subs-specific tagline.
...
**Fix:** parameterize the doc-name list via `blueprint.config.ts` → `documents: { prd: 'PRD.md', brd: 'BRD.md', ... }`. Default to bc-subs shape. Initiatives that use different filenames override.

### 6. Hive integration (substrate-specific)

`src/lib/derived.ts` and `src/components/DerivedRoadmap.tsx` read `docs/audits/_state.json` (bc-subs' state-derive output) and Hive substrate data.
...
`src/components/SubstrateDashboards.tsx` + `src/pages/inspect/gates.astro`, `coverage.astro`, `dependencies.astro`, `attestations.astro` are bc-subs-specific governance views.

**Fix:** these are advanced Tier-2 features. For Tier 1 initiatives, ship `/inspect` as a single methodology overview page that links to `docs/decisions/` (ADRs). Move the bc-subs-specific views to `@blueprint/ui-substrate-hive` as an optional add-on package.
...
5. Rename `public/subs-logo.png` → `public/project-logo.png` and add a placeholder.
6. Gate substrate-specific Inspect pages behind `substrate !== 'none'`.

After step 5, the scaffold passes `grep -L "bc-subscriptions" .` for everything except inline `bc-subscriptions` references in the README (which is a separate de-bcization pass on the doc).
...
- Extracted via `git archive` from `bc-subscriptions@origin/main` on 2026-05-25
- Source commits: see bc-subscriptions git log for `apps/portal/`, `packages/ui/`, `packages/design-tokens/`
