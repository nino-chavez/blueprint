...
**Promotion shape (wave 25)**: new canonical doc `docs/patterns/operator-handoff-pattern.md` + new template subdir `template/methodology/handoff/` (with `README.md` + `handoff-template.md`) matching the existing `template/methodology/{design,voice}/` convention. No reviewer gate — both consumer examples are *successful* voluntary handoffs; the audit didn't surface failure-mode evidence to justify forcing a Stage N→N+1 reviewer. A future amendment can add the reviewer when ≥2 consumers report stage-transition failures attributable to missing handoffs.
