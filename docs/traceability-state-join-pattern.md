...
**Extracted from:** `bc-subscriptions` session 2026-05-16.
...
This pattern is only as good as the `_state.json` refresh cadence. In `bc-subscriptions`, state-derive runs on every push to main via `derive-state-on-main.yml`. The registry generator also runs on every push (via a separate workflow). Both outputs commit back to the repo so the traceability matrix at `subs-prototype.pages.dev/traceability` always reflects HEAD.
