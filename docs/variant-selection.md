...
the original employer-prefixed name serves three distinct project lifecycles. Each has its own stage sequence, deliverables, and reviewer-agent gates. Pick the variant *before* the first stage runs — the wrong variant produces retrofit feel that cannot be un-retrofitted without restarting.
