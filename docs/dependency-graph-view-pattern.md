...
**Extracted from:** `bc-subscriptions` session 2026-05-20 — ships as the
third tab of the `subs-status` Cloudflare Pages deploy (alongside
...
| Layer       | What it does                                                                 | bc-subscriptions location                       |
|-------------|------------------------------------------------------------------------------|-------------------------------------------------|
...
bc-subscriptions is the first adopter. Three layers can flow into
...
One swap required: the hardcoded GitHub repo URL (`nino-chavez/bc-subscriptions`)
...
1. **Where does the React primitive live?** bc-subscriptions has it in
...
2. **What data does the route fetch?** Static JSON baked at CI time (current
   bc-subs approach via `dep-graph-dashboard`) or live-fetch from a Worker
...
## Reference implementation (bc-subscriptions)
...
Live surface: `/dependencies` tab on the `subs-status` Cloudflare Pages
