...
**Source:** `bc-subscriptions` Hive #929 doc-reorg session (May 2026). The pattern emerged in a single Opus session that went from "I think it's time to clean up the docs" to a filed 13-PR spec ready for autonomous execution.
...
`bc-subscriptions` Hive #929 (2026-05-16):
...
Pattern extracted from `bc-subscriptions` Hive #929 doc-reorg session (2026-05-16). The session is the canonical reference implementation.
