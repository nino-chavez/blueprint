...
Canonical voice reference for the original employer-prefixed name deliverables. Loaded on demand; not inlined into per-initiative `CLAUDE.md`. Reviewer agents (`doc-quality-auditor`, `terminology-linter`) enforce the rules in this doc.
...
If `signal_forge.enabled: true`, the voice guides at `forge-signal/docs/voice/` are authoritative and this doc defers to them. Otherwise these rules apply.
...
Distilled from the a commerce-platform pricing & packaging CX initiative (March 2026) and refined across Rally HQ, website-nc-v3, and Signal Dispatch blog audits. Moved out of `template/CLAUDE.md` 2026-05-25 per the context-efficiency sweep — voice rules are too long to inline per-session.
