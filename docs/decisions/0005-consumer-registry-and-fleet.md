...
# ADR-0005 — Consumer registry (`consumers.yml`) + `blueprint fleet` + `blueprint upgrade`
...
## Upgrade — the DOWN channel (`blueprint upgrade`, 2026-06-05)

`fleet` is the methodology-side READ of the whole fleet; `blueprint upgrade` is the consumer-side WRITE for one consumer — the DOWN channel of the bidirectional update protocol. It runs **inside a consumer repo**, reads THIS consumer's `methodology_version`, and — gated — bumps it toward current.

**It is `fleet` for N=1.** `upgrade` builds a synthetic one-entry registry and calls the SAME `classifyConsumer` (the version model is not reinvented). The `breaking` determination is therefore a **field on the shared classifier** (so `fleet` and `upgrade` read one verdict): `true` when a `behind` pin crosses a major boundary (semver: current major > pin major; sha: `package.json` major increased across the range), `false` within the same major, `null` (UNKNOWN) when a sha pin predates `package.json` so the major can't be read.

**plan/apply split (Terraform canonical).** Default `blueprint upgrade` is a DRY-RUN preview: classify, narrate the delta, print the proposed bump + the gate decision, **mutate nothing**. `--apply` performs the write. This is the only canonical-shaped choice for a mutate-a-repo command.

**The delta narration is honest, never fabricated.** A semver pin slices the `## <semver>` sections of `CHANGELOG.md` in `(pin, current]`. A sha pin (today's only real case — `010945a` predates the `0.1.0` baseline, so no released sections sit in range) degrades to the methodology **commit log** between pin and HEAD, explicitly labeled "no semver releases in range." It never invents a CHANGELOG entry; `CHANGELOG.md` is regenerated by Changesets (ADR-0007) and only read here.

**The apply gate (safe-by-default):**
- `breaking === true` (major bump) → **refuse**; review the migration note, bump by hand.
- `breaking === null` AND a sha range with zero semver tags (today's path) → **refuse** unless `--ack-untagged`: breaking-ness is genuinely UNKNOWN, so silent auto-apply over a range full of `feat(template)`/`refactor(template)` commits would be the opposite of safe. The operator acknowledges explicitly.
- `class=ahead` / `unresolvable` → refuse (can't compute a safe forward target).
- `class=behind`, `breaking===false` → apply the pin bump. `class=unpinned` → with `--apply`, insert a first pin (to the stable semver release identity when one exists, else short HEAD). `class=current` → noop.

**Reversibility is real, not asserted.** Before any `--apply` mutation, a **dirty-tree guard** refuses if `blueprint.yml` has uncommitted changes (else the `git checkout` revert hint would discard them). The write is a single anchored, column-0 `methodology_version` line (a function-replacement splice that can't be backreference-injected; never touches `methodology_home`), staged to a temp file and **renamed into place only after the value round-trips** — a failed write leaves the original pristine (`blueprint.yml` is evidence; a half-write is a durability violation). The revert hint is conditional on tracked state.

**v1 is pin-bump only.** Chrome re-stamp stays the separate, explicit `stamp.mjs --mode=restamp-chrome` step — bundling it added the heaviest surface and the `--accept-overwrite=ALL` footgun for a capability that already exists standalone. `upgrade` prints a pointer; re-stamp orchestration is deferred.

**Exit codes:** 0 = current, a successful `--apply`, or unpinned (informational; `--require-pin` flips unpinned to 1 for CI staleness gating of pinned consumers); 1 = drift not applied (behind/ahead/unresolvable, or a refused breaking/untagged bump); 2 = no `blueprint.yml` at target, dirty tree on `--apply`, or a write/round-trip failure.
...
- **`blueprint upgrade --restamp`** — orchestrate `stamp.mjs --mode=restamp-chrome` from upgrade (run-first ordering, per-file `--accept-overwrite` from a dry-run `audit-chrome`, partial-write recovery). Deferred from the step-8 v1 (pin-bump only) to keep the write surface minimal and avoid the `--accept-overwrite=ALL` footgun.
- **`--ack-untagged` is a v1 stopgap** for the untagged-sha-range breaking-UNKNOWN case. Once `vX.Y.Z` tags exist between real pins and HEAD, the semver major-boundary check fires and this becomes the explicit escape hatch for never-tagged fork shas — decide at first-tag whether to keep it.
