...
Pattern A portals expose an audience switcher with three pills that reorder the home-lane priority. `subs-initiative` introduced them as **executive / discovery / internal**, and the names propagated into `template/apps/portal/` and `template/packages/ui/` when that shell was lifted into the methodology repo on 2026-05-25.
...
Default audience: `evaluator` (was `discovery`). Storage key (`bcs-audience`) and change event (`bcs-audience-change`) keep their subs-initiative-era prefixes for now — separate cleanup, tracked below.
...
3. **`engineering` is honest.** The "internal" audience in subs-initiative is engineers + methodology folks reviewing the build. Naming them by their actual role removes the question.
...
- **Storage key prefix `bcs-`**: `bcs-audience` and `bcs-audience-change` still carry the subs-initiative prefix from the original lift. Rename to `blueprint-audience` and `blueprint-audience-change` in a separate change, since it's a different concern (template-prefix cleanup, not pill naming). Tracked as ADR-0002 when picked up.
