...
Canonical design tokens for the bc-subscriptions family — OKLCH color, type scale, spacing, motion. Framework-agnostic; consumed by React (`packages/ui`), Svelte (`apps/storefront-svelte`), vanilla HTML (`apps/demos`), and any future surface that joins the family.
...
Change `--bcs-brand-lch` once and every derived shade re-themes automatically. The same idiom is used in BigCommerce's storefront-kit; we intentionally rhyme.
...
All variables are prefixed `--bcs-*` so they never collide with merchant-supplied tokens on the storefront, BigDesign tokens in the admin marketplace shell, or BC platform tokens elsewhere.
