...
# `source` (repo-relative path, resolved against the initiative's parent
# directory) into _docs/<id>.md so the canonical viewer (docs/index.html) and
# the chat function (functions/api/chat.js) can serve them from the deploy root.
#
# Manifest-driven; no hardcoded SOURCES table. Entries without a `source` field
# are assumed to already exist at _docs/<id>.md (consumers may author directly
# there when no canonical path applies).
#
...
BLUEPRINT_DIR="$(cd "$SCRIPTS_DIR/.." && pwd)"
INITIATIVE_DIR="$(cd "$BLUEPRINT_DIR/.." && pwd)"
DOCS_OUT="$BLUEPRINT_DIR/_docs"
MANIFEST="$BLUEPRINT_DIR/_meta/index.json"
