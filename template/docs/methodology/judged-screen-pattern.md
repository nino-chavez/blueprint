...
**Provenance.** These four findings are relayed from the Minder promotion brief, not resolved against Minder's own files from inside this repo. They are reported rather than quoted for that reason. A consumer citing this section for its own amendment should resolve them at source first — which is the rule § 8 says this pattern extends rather than contradicts.
