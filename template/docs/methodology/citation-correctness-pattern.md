...
1. **Mechanical gate.** The citation lint walks markdown, extracts all `http(s)://` citations, and reports any that return an HTTP error status or fail to connect (with a 7-day cache to avoid re-checking every run). It ships at `$BLUEPRINT_HOME/template/tools/cited-url-lint/`, and the stamper does not copy it into an initiative. Run that copy by absolute path from the initiative root, once per directory of cited markdown: `npx tsx "$BLUEPRINT_HOME/template/tools/cited-url-lint/index.ts" research --fail-on-empty`. With `--fail-on-empty`, a scan that checked nothing exits 3 instead of 0. `fact-check-loop-reviewer.md` § Running the citation lint says how to read the output.
2. **No self-attestation.** When a reviewer receives a claim of the form "verified against source X," the reviewer's job is **not** to re-read the artifact's verification note. It is to resolve the URL and check it directly. If the lint has already run and the URL passes, the reviewer can trust the lint result. If the lint failed, the violation stands. A run that checked nothing has passed nothing: a lint result counts only when its first `cited-url-lint:` line names the directory that holds the citations and its last one reads `clean — N unique URLs resolved`. `nothing was checked`, or a count of URLs not checked, is not a pass.
3. **Allowlist for edge cases.** URLs that require authentication, are behind anti-bot protection, or are legitimately cited as examples (code-block paths, internal URLs) can be allowlisted in `.cited-url-lint-allowlist` with a comment explaining why. The lint reads that file from the working directory, so keep it at the initiative root. Allowlisting every citation leaves nothing to check, and the lint says `nothing was checked`, not `clean`.
