...
- **Derive-on-main, never hand-commit.** The register refreshes via CI on every push to the main/dev branch (the `derive-state-on-main` pattern in `docs/traceability-state-join-pattern.md` § Operationalizing). Derived outputs are automation-owned; a gate blocks hand commits.
...
2. **Per-AC ↔ scenario linkage** in the traceability registry (candidate fields: `acVerified`, `scenarioStatus`) and the join-key contract that keeps it drift-resistant — the same discipline as the `BRD.md §US-X.Y:` reference prefix in `docs/traceability-state-join-pattern.md`.
...
- `docs/traceability-state-join-pattern.md` relabels its `implementationStatus` join as a G3 (presence) signal.
