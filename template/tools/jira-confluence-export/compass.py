...
def emit_components() -> list[dict]:
    ...
    for d in sorted(p for p in APPS_DIR.iterdir() if p.is_dir()):
        ...
        comps.append({
            ...
            "description": f"bc-subscriptions {name} surface",
            "ownerId": "<team-id>",                       # operator fills (Compass team)
            "links": [{"type": "REPOSITORY",
                       "url": "https://github.com/nino-chavez/bc-subscriptions",
                       "name": f"apps/{name}"}],
            "labels": ["bc-subscriptions"],
        })
    return comps
...
def emit_scorecard() -> dict:
    ...
    return {
        ...
        "description": "bc-subscriptions per-epic launch-readiness gates "
