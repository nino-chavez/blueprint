...
default (the connected MCP instance is bigcommercecloud.atlassian.net; this
    ...
    for hint in sorted({p["space_hint"] for p in pages}):
        ...
        res = client.post("/wiki/api/v2/pages", {
            ...
            "body": {"representation": "storage",
                     "value": f"<p>{_esc(title)} — derived from the bc-subscriptions "
...
def main() -> int:
    ...
    if args.execute:
        ...
        if missing:
            ...
            return 2
        if "bigcommercecloud" in site:
            sys.stderr.write("[error] refusing to import synthetic data into corporate prod "
                             "(bigcommercecloud.atlassian.net). Use a free sandbox site.\n")
            return 2
