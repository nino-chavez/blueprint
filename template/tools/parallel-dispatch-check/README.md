...
See `~/Workspace/dev/wip/blueprint/docs/multi-operator-collab-pattern.md` § "What consumers can do today" for the full lightweight-convention set (per-repo role declaration + worktree isolation + no-workaround success criterion). This tool addresses the worktree-isolation convention specifically.
