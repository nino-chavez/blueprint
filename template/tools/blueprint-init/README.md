...
Mechanically-checkable scaffold for a Tier 1 Blueprint portal. Replaces the previous "copy `template/apps/portal/` and remember to de-bc-ize" pattern that left `subs-initiative` strings embedded in 6+ files (per `template/apps/portal/HANDOFF-DEBCIZATION.md`).
...
|---|---|---|
| `--name` | yes | Project slug. Substitutes `subs-initiative` in code, package descriptions, source links. |
| `--display-name` | yes | Human-facing brand string (footer, PortalNav, page titles). |
| `--repo-url` | yes | Full GitHub URL (`https://github.com/owner/repo`). Substitutes `the subscriptions initiative's repo (private)`. |
| `--tagline` | yes | Footer tagline. Substitutes `An example product initiative`. |
