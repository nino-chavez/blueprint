 ...
 * pattern that left bc-subscriptions strings embedded in 6+ files. The stamper
...
function substitutions({ name, displayName, repoUrl, tagline, theme }) {
  ...
  return [
    ...
    { from: "https://github.com/nino-chavez/bc-subscriptions", to: repoUrl },
    { from: "BigCommerce-native subscription management", to: tagline },
    ...
    // don't inherit bc-subscriptions' substrate endpoint. The component renders
    // a disabled "substrate not yet configured" state when the URL is empty.
    { from: "https://subs-archaeology.bigcommerce-testing-7727.workers.dev", to: "" },
  ];
}

// Files where the bc-subs strings are business content (10-gate framework,
...
const BANNER_LINES = {
  ".astro": [
    ...
    "  from the bc-subscriptions reference initiative. Rewrite or delete before sharing",
...
async function renameLogo(target, dryRun, log) {
  const src = path.join(target, "apps/portal/public/subs-logo.png");
  const dst = path.join(target, "apps/portal/public/project-logo.png");
  const exists = await fs.stat(src).catch(() => null);
  if (!exists) return;
  if (dryRun) {
    log.renamed.push("apps/portal/public/subs-logo.png → project-logo.png");
    return;
  }
...
async function mechanicalCheck({ target, log }) {
  ...
  const tripwires = [
    "bc-subscriptions",
    "BigCommerce-native subscription management",
    "/subs-logo.png",
  ];
