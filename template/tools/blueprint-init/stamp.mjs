...
async function writeBlueprintYml({ target, name, variant, tier, portalType, tagline, repoUrl, dryRun, log }) {
  ...
  const portalBody = [
    ...
    "",
    "# ──────────────────────────────────────────────────────────────────",
    "# Design intent (judged-screen-pattern.md § 2a — promotion candidate).",
    "# How much design work this change is; decides what Stage 2 owes.",
    "#   preserve = approved direction stands  -> design_direction required below",
    "#   refit    = change presentation, keep behavior -> EXPERIENCE-BRIEF.md required",
    "#   rethink  = the direction is the question -> brief + 3 concepts + selection ADR",
    "# Stamped EMPTY on purpose: undeclared WARNs, and is not treated as preserve.",
    "# ──────────────────────────────────────────────────────────────────",
    'design_intent: ""                 # preserve | refit | rethink',
    'design_direction: ""              # repo-relative path to the approved direction',
    '                                  # record; REQUIRED when design_intent: preserve',
    'screen_review_policy: ""          # "" (warn) | strict — screen-composition-reviewer',
    "",
