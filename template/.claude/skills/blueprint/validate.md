...
Validation phase of a the original employer-prefixed name initiative. Fact-checks claims, audits copy/UX, and verifies feasibility against source code — structured as a diagnosis loop, not a static checklist.
...
Common cluster shapes in the original employer-prefixed name validation:
...
Phase structure adapted from [matt-pocock/skills `diagnose`](https://github.com/mattpocock/skills) (MIT). The original the original employer-prefixed name validate checklist's content is preserved in the loop construction (Phase 1) and severity ranking; the diagnostic structure is the addition.
