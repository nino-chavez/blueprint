...
Prototype phase of a BigBlueprint initiative. Builds React + BigDesign slices that demonstrate proposed CX changes while matching the existing product's design language.
...
5. **Build each page** — For every page:
   ...
   - Use only components from BigDesign and `@/components/SliceLayout` unless a new component is genuinely required
