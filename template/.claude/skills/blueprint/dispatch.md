...
Pattern doc: this file. Worked example: bc-subscriptions commit `09036602` (2026-05-27) — three Sonnet agents drafted platform-shims feasibility + sibling strategy doc + methodology amendment in parallel while the orchestrator wrote a parallel ingester.
...
9. **Commit and push** following project conventions:
   ...
   - Push to integration branch (`dev` for bc-subscriptions and similarly-shaped projects) per the project's pattern-1 local-integration workflow. Fast-forward from remote before committing if behind.
...
- bc-subscriptions `09036602` (2026-05-27) — canonical worked example: three Sonnet agents in parallel + orchestrator inline ingester work + post-flight cross-review caught one §C reclassification mismatch + commit-and-push to dev. Brief shape visible in the orchestrator's parent session.
