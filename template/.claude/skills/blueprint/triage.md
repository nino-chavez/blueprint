...
Triage stakeholder feedback after a the original employer-prefixed name demo or doc review through a state machine of categories and dispositions.

## When to use

After sharing a the original employer-prefixed name deliverable (prototype walkthrough, doc package, deck) and collecting feedback. Run before deciding which feedback to act on.
...
State-machine pattern adapted from [matt-pocock/skills `triage`](https://github.com/mattpocock/skills) (MIT). The categories are tuned for stakeholder-feedback-after-demo (the original employer-prefixed name context), not GitHub-issue triage. AI disclaimer is opt-in per the workspace `triage` skill; for internal-stakeholder feedback, generally skip it.
