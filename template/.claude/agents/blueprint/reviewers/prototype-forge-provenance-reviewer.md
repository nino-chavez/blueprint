...
description: Stage 3 completion gate. Two checks — (A) forge-pipeline evidence files present (brand-kit, signal-forge bridge, gen-images output, forge-site archetype reference) when a substantive prototype exists, and (B) every Stage 1 JTBD has a corresponding prototype surface that COULD satisfy its acceptance criteria. Encoded response to ADR-0004 — prevents hand-built-from-LLM-aesthetic-instincts prototypes that bypass the deterministic generation layer.
...
3. **Scan for evidence files per forge tool.** For each tool, look for one or more expected artifacts:
   ...
   | `forge-signal` | bridge YAML (typically `brand-kit/signal-forge.yml` or `bridges/signal-forge.yml`), generated copy output under `content/` |
