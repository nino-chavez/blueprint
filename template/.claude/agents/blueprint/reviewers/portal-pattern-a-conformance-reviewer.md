...
Any match BLOCKS at the Stage 3 → Stage 4 gate. The banner is intentionally stamped onto bc-subs-content-carrying files by `template/tools/blueprint-init/stamp.mjs`; the operator's job is to delete the banner (and replace or strip the surrounding content) before the portal goes to stakeholders. A passing portal has zero `REPLACE_FOR_PROJECT` markers.
