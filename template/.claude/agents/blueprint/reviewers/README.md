...
| `screen-composition-reviewer` | Any rendered surface declared ready; rerun on each build that changes a declared surface | All initiatives with a rendered surface (skips when no `EXPERIENCE-BRIEF.md` declares one) |
...
- **A judged gate is not a mechanical one, and must not pretend to be.** `screen-composition-reviewer` splits in two: the `.mjs` verifies a cold screen review is *recorded* for every declared surface; the `.md` is the protocol for the review itself, run by a second model, session, or person. The mechanical half can only check paperwork — a well-formed record asserting a judgment nobody made passes it. That is the honest ceiling of a static check on a judged artifact, not a defect (`docs/methodology/judged-screen-pattern.md` § 3).
