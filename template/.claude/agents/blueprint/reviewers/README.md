...
| `screen-composition-reviewer` | Any rendered surface declared ready; rerun on each build that changes a declared surface | All initiatives with a rendered surface (skips when no `EXPERIENCE-BRIEF.md` declares one). Which reviews it requires is set by `design_intent` |
...
| `encounter-audit-reviewer` | Any user-facing surface declared ready; rerun after copy-bearing config or generator changes | All |
...
- **Block on failure.** A reviewer's verdict is binary — PASS or BLOCKED. The calling agent must resolve all findings before re-invoking. Two exceptions, for different reasons: `defrag-reviewer` is WARN-only by design — it is a cadence coherence pass, not a gate, and blocking on drift would punish in-flight work for consolidation it hasn't had a chance to do yet. `screen-composition-reviewer` is WARN by default because its pattern is a **promotion candidate** — it promotes when a second product has run the pattern end to end; a consumer that wants blocking sooner opts in per initiative with `screen_review_policy: strict` in `blueprint.yml`. (`design-principles-reviewer` checks 10 and 11 are WARN for the same promotion-candidate reason, inside an otherwise blocking reviewer.) The strict opt-in is a declared scalar rather than a CLI flag because `blueprint review` forwards no argv to a reviewer.
...
- **A judged gate is not a mechanical one, and must not pretend to be.** `screen-composition-reviewer` splits in two: the `.mjs` verifies the required screen reviews are *recorded* per declared surface; the `.md` is the protocol for the reviews themselves, run by a second model, session, or person. The mechanical half can only check paperwork — a well-formed record asserting a judgment nobody made passes it. That is the honest ceiling of a static check on a judged artifact, not a defect (`docs/methodology/judged-screen-pattern.md` § 3).
- **Judging on sight and judging against a direction are two reviews.** A blind cold review asks whether the screen works; a direction-conformance review asks whether the team built the idea it selected. Neither proves the other, and the cold reviewer is disqualified from the conformance pass the moment they read the direction record — so cold runs first. One reviewer holding both questions is not cold.
...
- **Encounter gates beat source-only copy claims.** `terminology-linter` inspects authoring sources; `encounter-audit-reviewer` reads `reader-contract.json`, verifies the source map, and audits rendered output. A source rewrite is not complete until the copy survives the generator and makes sense on the surface a reader actually meets.
