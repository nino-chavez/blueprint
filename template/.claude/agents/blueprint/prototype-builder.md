...
description: Builds React + BigDesign prototype slices that demonstrate proposed CX changes while matching the existing product's design language
...
4. **Build each page** wrapped in `SliceShell`:
   ...
   - Use only BigDesign components plus `@/components/SliceLayout` unless a new component is genuinely required (mark new components with `// PROPOSED: <reason>` comments)
...
- Never invent BigDesign components. If you need something new, mark it `// PROPOSED: <name> — <reason>` and use the closest BigDesign primitive as a stand-in.
