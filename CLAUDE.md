...
Operator check before editing `template/`: confirm which external consumer initiatives are in flight and whether the edit has waiver authority. Editing the root self-application never trips the freeze. The consumer registry is `consumers.yml` (12 registered; inspect with `blueprint fleet`) — external initiatives at rest (subs-initiative mid-flight on its own pinned methodology copy — not mid-migration); verify with `blueprint fleet` rather than trusting this sentence.
