...
**Current state:** wave 31 logged (`5e7ede7` — Pattern A portal generic-harness refactor + semver baseline + portable BLUEPRINT_HOME; from the `wip/blueprint-platform` productization dogfood). blueprint-redesign at rest (`cc4f62f`); rally-hq at rest (`1aa3c35`); blueprint-platform active (scope ceiling A — methodology-native; design + a rendering Pattern A portal). Pattern A consumers re-stamp to inherit the generic harness + author a `blueprint.yml portal:` block (Tier-0 default builds clean without one). Methodology freeze lifted. Pattern B consumer-sync (waves 8-16): `--mode=audit-chrome` to classify divergences, then `--mode=restamp-chrome --accept-overwrite=<LAG-files>` to pull forward safely.
