...
**Current state:** wave 41 logged (`96f60db` — org-authored reviewer discovery / native extensibility, build-order step 11, ADR-0006; `reviewer-registry.mjs` discovers canonical + org-local + org-npm reviewers with canonical-wins shadow precedence, `blueprint review --list` + cross-source name resolution). Recent: wave 40 (`42369ab`) amendment/RFC triage classifier (UP channel); wave 39 (`ab4e9e2`) access + governance (operator action to bind: `docs/governance/apply-ruleset.sh`); wave 38 (`3d3904b`) `blueprint upgrade` DOWN channel; wave 37 (`1bfdc19`) consumer registry + `blueprint fleet`; wave 36 (`d95dadd`) cost telemetry + sweep + gate; wave 35 (`eb250e5`) cost dial. From the `wip/blueprint-platform` productization dogfood (scope ceiling A — methodology-native; design + a **deployed + public** Pattern A portal at blueprint-platform.pages.dev / github.com/nino-chavez/blueprint-platform; build-order steps 0–11 done, step 12 = `blueprint doctor` (conformance/health gating on runtime/browser verification — the false-green guard) next, which completes the build order). blueprint-redesign at rest (`cc4f62f`); rally-hq at rest (`1aa3c35`); blueprint-platform active. Pattern A consumers re-stamp to inherit the generic harness + author a `blueprint.yml portal:` block (Tier-0 default builds clean without one); `npx @nino-chavez/blueprint-cli init` is the scaffolder once published. Methodology freeze lifted. Pattern B consumer-sync (waves 8-16): `--mode=audit-chrome` to classify divergences, then `--mode=restamp-chrome --accept-overwrite=<LAG-files>` to pull forward safely.
