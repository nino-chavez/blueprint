...
2. **Blueprint applied to itself** (the self-application / reference presentation): a root `blueprint.yml`, `research/`, `decisions/`, `feedback/` (stakeholder captures + triage; demand-evidence log at `docs/content/validation-script.md`), `apps/portal/`, `packages/`, `tools/archaeology/` — the productization initiative, run *as* a Blueprint initiative, in this repo. The bespoke portal at `apps/portal/` is the live demo and onboarding knowledge base for new consumers; it is one actor-specific output, not a required presentation for every initiative.
...
Operator check before editing `template/`: confirm which external consumer initiatives are in flight and whether the edit has waiver authority. Editing the root self-application never trips the freeze. The consumer registry is `consumers.yml` (15 registered; inspect with `blueprint fleet`) — external initiatives at rest (subs-initiative mid-flight on its own pinned methodology copy — not mid-migration); verify with `blueprint fleet` rather than trusting this sentence.
...
**Standing state:** build order 0–13 COMPLETE across tracks A–E. The CLI is **PUBLISHED** — `@nino-chavez-labs/blueprint-cli` on npm (MIT, public repo; `npm view` for the live version — releases ship via `bin/release-if-unpublished.mjs` on main pushes); `npx @nino-chavez-labs/blueprint-cli init` is the public scaffolder and `init/review/cost/fleet/upgrade/doctor/hive setup/stage/feedback` are all real (thirteen dependency-free libs under `template/tools/lib/` + the hive bootstrap, each with self-tests). The `main-protection` ruleset is bound + active (2026-06-05, id 17343422 — PR + 1 code-owner review + linear history, admin-bypass during the solo phase). Reviewer fleet: see `blueprint review --list` for the executable set and the spec directory for the full roster; `doctor` runs its baseline checks plus declared-capability checks. Consumers are registered in `consumers.yml` (inspect with `blueprint fleet`); external initiatives at rest. Pattern A consumers re-stamp to inherit the generic harness and declare real reader outputs in `actor-output.yml`; Pattern B consumer-sync: `--mode=audit-chrome` to classify divergences, then `--mode=restamp-chrome --accept-overwrite=<LAG-files>`.
