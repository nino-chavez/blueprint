...
**Current state:** wave 33 logged (`66e2c51` — `loadExcerpt` clean-prose fix). Recent: wave 32 (`5ced121`) `@nino-chavez/blueprint-cli` dispatcher + resolver (build-order step 2); wave 31 Pattern A generic-harness + semver baseline. From the `wip/blueprint-platform` productization dogfood (scope ceiling A — methodology-native; design + a **deployed** Pattern A portal at blueprint-platform.pages.dev; build-order steps 0–2 done, step 3 = first `.mjs` reviewer next). blueprint-redesign at rest (`cc4f62f`); rally-hq at rest (`1aa3c35`); blueprint-platform active. Pattern A consumers re-stamp to inherit the generic harness + author a `blueprint.yml portal:` block (Tier-0 default builds clean without one); `npx @nino-chavez/blueprint-cli init` is the scaffolder once published. Methodology freeze lifted. Pattern B consumer-sync (waves 8-16): `--mode=audit-chrome` to classify divergences, then `--mode=restamp-chrome --accept-overwrite=<LAG-files>` to pull forward safely.
