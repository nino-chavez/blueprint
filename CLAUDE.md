...
The blueprint-redesign consumer initiative is at rest as of `dogfood-v1` (commit `cc4f62f` on `dogfood/self-redesign`). The methodology freeze that was in effect during its in-flight period has lifted; methodology can now advance in parallel with future consumer initiatives. Consumer-sync command for existing Pattern B portals to pull waves 8-12: `node template/tools/blueprint-init/stamp.mjs --mode=restamp-chrome --pattern=B --target=<portal-dir> --theme=<slate|coral|forest|minimal>`.
