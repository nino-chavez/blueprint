...
- Wave 19 — Enforces what wave 17 declared. Wave 17's `stamp.mjs` upgraded the `REPLACE_FOR_PROJECT` banner text from "warning" to "block" but the portal-pattern-{a,b}-conformance-reviewers never grep'd for the banner — declaration without enforcement. Adds new check section to both reviewers: grep `REPLACE_FOR_PROJECT` across the portal source; `>0` matches BLOCKS at the Stage 3 → Stage 4 gate. Each reviewer's STATUS report block gains `REPLACE_FOR_PROJECT_BANNERS` line. Without wave 19, the bc-promo-rules failure mode (scaffold pages with bc-subs business content shipping to live stakeholder deploy) could recur — the banner text says "block" but nothing blocks.
