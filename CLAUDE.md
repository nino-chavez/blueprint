...
**Current state:** wave 54 — **docs/ reorg (typed subdirs, full clean tree)** — `docs/` top level reduced from 41 loose files to the two SessionStart-hook-injected canonicals (`variant-selection.md`, `portal-and-tier-ladder.md`) + `README.md`; 38 files moved into `docs/patterns|context|case-studies|governance|_archive/`; references rewritten repo-wide incl. `template/`; doc-currency green. BREAKING for stamped consumers (stale `$BLUEPRINT_HOME/docs/<file>` paths in their local CLAUDE.md until re-stamp or the pick-up-updates prompt). Prior: wave 53 — **Mom Test ask-outcome discipline** (`template/docs/methodology/mom-test-validation-pattern.md` § Ask outcomes: taken / not taken / no-ask-made; an untaken ask is first-class negative evidence; `/blueprint-triage` records outcomes; self-app's `docs/content/validation-script.md` Log re-columned + NEW A6 personal-incumbent-displacement assumption). Prior: wave 52 — **DoD verification ladder, presence ≠ function** (`template/docs/methodology/dod-verification-ladder-pattern.md`: five gates per AC; state-derive relabeled as a presence oracle; subs-initiative designs the mechanical half). **Full wave history: `WAVE-LOG.md`** — filter with `node template/tools/wave-digest/digest.mjs --source=WAVE-LOG.md --since=N`.
