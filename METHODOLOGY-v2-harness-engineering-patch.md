...
**Status**: Increment 1 (Stage 0) **LANDED in `METHODOLOGY.md` 2026-05-25** — this doc is the historical proposal + the deferred increments. Stage 0 is now canonical; the section below describes what got promoted. Increment 2 (Ralph Wiggum Loop) was reshaped by the v3 variant taxonomy and the reviewer agents now live at `template/.claude/agents/blueprint/reviewers/`. Architectural invariants + lint-as-remediation-prompts (originally Increment 3 side items) **LANDED in `template/prototype/DESIGN.md` 2026-05-25** as the "Architectural Invariants" section. "Agent struggle is a missing capability" **LANDED in `METHODOLOGY.md` 2026-05-25** as the "First Principle" section. Increment 3 map-not-manual restructure (AGENTS.md + docs/ tree + janitor) remains deferred — lowest urgency, only bites large initiatives.
...
The Codex experiment surfaced primitives the original employer-prefixed name either lacks or does manually. The gap that matters: the original employer-prefixed name's agent can produce a deployed prototype but cannot *validate its own work in the browser*, cannot run a closed convergence loop against named reviewers, and cannot encode lint failures as remediation prompts. Stakeholder polish suffers as a result — fact-check and iterate stages stall on human attention.
...
Conversation 2026-05-25 reviewing the OpenAI harness engineering post against current the original employer-prefixed name methodology. PDF source: `/Users/nino/Downloads/Harness engineering_ leveraging Codex in an agent-first world _ OpenAI.pdf`.
