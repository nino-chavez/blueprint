...
| Copy-paste of `template/apps/portal/` left `bc-subscriptions` strings embedded in 6+ files | `template/tools/blueprint-init/stamp.mjs` stamps a fresh portal with mechanical post-stamp grep |
...
See `template/apps/demos/README.md` for the full setup recipe. Origin: extracted from `bc-subscriptions` (May 2026) where it answered the multi-surface demo problem across Stencil / Catalyst / custom-headless / BC Admin.
...
### Stage S-A: Archaeology Substrate (added 2026-05-27 wave 21 — bc-promo-rules amendment)
...
**Why this is a stage, not a cross-cutting discipline**: the substrate has a discrete linear lifecycle (scaffold → deploy → ingest → flip UI flag) with distinct Done-criteria at each step, and the wrong choreography produces the bc-promo-rules failure mode where a portal ships with the "Ask the substrate" widget live but pointed at a non-existent or wrong-project substrate.
...
**Why the gates exist**: bc-promo-rules (May 2026) deployed a live "Ask the substrate" widget pointed at the bc-subscriptions reference Worker because the canonical Layout stamped the component unconditionally AND the component's `WORKER_URL` was hardcoded to bc-subs. The widget surfaced bc-subs suggested questions ("Why did we reject terraform-gcp-platform?") to bc-promo-rules stakeholders. Waves 17 + 18 closed the leak at three layers; this stage codifies the correct lifecycle so future consumers know when the operator flag flip is earned.
...
| **signal-forge** | AI-powered document generation (when blog/thought-leadership voice is wanted) | External tool |
| **bc-figma-generator** | Design asset generation from Figma specs | External tool |
...
Originally **BigBlueprint**, following BigCommerce tooling conventions (BigDesign, BigTools). Renamed to **Blueprint** on 2026-05-25 once the methodology proved project-agnostic across Rally HQ, Signal Dispatch, website-nc-v3, TNA, and the original BC work.
...
These disciplines emerged from `bc-subscriptions` (May 2026) and the rally-hq 17-meta fan-out (May 2026). When they apply, they belong cross-cutting (not as a pipeline stage) — the pipeline produces the deliverables; the disciplines keep the surface around the deliverables coherent.
...
Extracted from the BigCommerce Pricing & Packaging CX initiative (March 2026). The full project is at `/Users/nino.chavez/Workspace/dev/wip/pricing-packaging/` with the deployed prototype at `prototype-cx.vercel.app`. Cross-cutting disciplines added May 2026 from `bc-subscriptions` doc-reorg work.
